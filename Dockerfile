...
FROM golang:1.26.2-alpine AS api-dev

RUN go install github.com/air-verse/air@latest

WORKDIR /repo
ENV GIN_MODE="debug"
CMD ["air", "--build.cmd", "go build -o bin/api cmd/api/main.go", "--build.entrypoint", "./bin/api", "--build.exclude_dir", "web,bin,deploy,tmp,e2e,docs"]
