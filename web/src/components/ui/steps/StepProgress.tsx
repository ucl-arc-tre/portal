...
function StepList({ steps, ariaLabel }: { steps: Step[]; ariaLabel?: string }) {
    ...
    <div className={styles["step-progress-container"]}>
      {isComplete ? (
        <details className={styles["completion-container"]}>
          ...
          <StepList steps={steps} ariaLabel={ariaLabel} />
        </details>
      ) : (
        <>
          <p className={styles["intro-text"]}>{introText}</p> <StepList steps={steps} ariaLabel={ariaLabel} />
        </>
      )}
    </div>
  );
